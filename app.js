...
function gamePage(g){
  ...
  return `<div class="shell game-page" data-theme="${esc(g.slug)}" style="--accent:${c1};--accent2:${c2}">
    <section class="game-hero" style="--game-banner:url('${bannerAsset(g.slug)}')"><div class="game-hero-content"><button class="back-btn" data-route="games">← Back to games</button><div class="game-identity">${gameImage(g,`${g.name} official logo`,"game-page-logo")}<div><span class="mini-label">GAME EXPERIENCE</span><h1>${esc(g.name)}</h1></div></div><p>${esc(g.tagline)}</p><div class="meta"><span class="pill">${esc(g.category)}</span><span class="pill">${esc(g.resource)}</span>${g.secondary?`<span class="pill">${esc(g.secondary)}</span>`:""}</div></div></section>
